(function () {
    ...
    gulp.task('swig', function () {
        var htmlDest = paths.target + 'views';
        var html = gulp.src(paths.html)
            .pipe($.newer(htmlDest))
            .pipe($.if(mini, $.minifyHtml()))
            .pipe(gulp.dest(htmlDest));
        var dest = paths.target + 'page';
        var swig = gulp.src([paths.swig, '!./**/_*.*'])
            .pipe($.newer({ dest: dest, ext: '.html'}))
            .pipe($.frontMatter())
            .pipe($.swig({
                defaults: {
            ...
            .pipe($.if(mini, $.minifyHtml()))
            .pipe(gulp.dest(dest));
        return merge(swig, html);
    });
    gulp.task('static', function () {
        var imgDest = paths.target + 'img';
        ...
        var css = gulp.src(paths.css)
            .pipe(iconfontFilter).pipe($.newer('iconfont/iconfont.css')).pipe($.concat('iconfont/iconfont.less')).pipe(iconfontFilter.restore)
    ...
    gulp.task('watch', ['dev'], function () {
        ...
        gulp.watch([paths.swig, paths.html], ['swig']);
